...
const logger = BlitzLogger({ name: "/api/demo/unarchive-all" })
...
const handler = async (request: NextApiRequest, response: NextApiResponse) => {
  ...
  if (!session.userId) {
    if (process.env.API_TOKEN && request.headers["api-token"] === process.env.API_TOKEN) {
      logger.info("Access to /api/demo/unarchive-all granted due to valid api token")
    } else {
      logger.warn("Access to /api/demo/unarchive-all denied")
